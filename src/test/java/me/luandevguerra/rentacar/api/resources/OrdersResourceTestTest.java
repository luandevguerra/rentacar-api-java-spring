...
import me.luandevguerra.rentacar.api.service.OrdersService;
...
class OrdersResourceTestTest extends AbstractResourceTest {
    ...
    void shouldReturnAListWithOrders() throws Exception {
        ArrayList<OrderDTO> orderList = DataTestProvider.getOrderDTOList();
        ...
        mockMvc.perform(get(getPath()).contentType(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$[0].date").value(DataTestProvider.getDateAsStringArray(orderList.get(0).getDate())))
                .andExpect(jsonPath("$[0].price").value(orderList.get(0).getPrice()))
                .andExpect(jsonPath("$[1].id").value(orderList.get(1).getId()))
                .andExpect(jsonPath("$[1].date").value(DataTestProvider.getDateAsStringArray(orderList.get(1).getDate())))
    ...
    void shouldReturn200CodeAndACar() throws Exception {
        long pathParam = 1L;
        OrderDTO order = DataTestProvider.getOrderDTO(pathParam);
        ...
        mockMvc.perform(get(getPath() + "/" + pathParam).contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.date").value(DataTestProvider.getDateAsStringArray(order.getDate())))
    ...
    void shouldReturn201CodeAndACarOnResponseOnceACarWasCreated() throws Exception {
        OrderDTO order = DataTestProvider.getOrderDTO(1L);
